...
public class PainelConsultarCliente extends JPanel implements ActionListener {
	...
	private JButton btnConcederEmprestimo;
	...
	public PainelConsultarCliente() {
		...
		btnConcederEmprestimo = new JButton("Conceder empr\u00E9stimo");
		sl_panelConta.putConstraint(SpringLayout.NORTH, btnConcederEmprestimo, 23, SpringLayout.SOUTH, depositarBtn);
		sl_panelConta.putConstraint(SpringLayout.WEST, btnConcederEmprestimo, 0, SpringLayout.WEST, depositarBtn);
		sl_panelConta.putConstraint(SpringLayout.SOUTH, btnConcederEmprestimo, 60, SpringLayout.SOUTH, depositarBtn);
		sl_panelConta.putConstraint(SpringLayout.EAST, btnConcederEmprestimo, 0, SpringLayout.EAST, depositarBtn);
		btnConcederEmprestimo.setPreferredSize(new Dimension(100, 25));
		btnConcederEmprestimo.setFont(new Font("Arial", Font.PLAIN, 25));
		btnConcederEmprestimo.setEnabled(false);
		btnConcederEmprestimo.setActionCommand("Conceder emprestimo");
		btnConcederEmprestimo.addActionListener(this);
		panelConta.add(btnConcederEmprestimo);
		sl_panelDadosPessoais.putConstraint(SpringLayout.NORTH, btnSalvarAlteraes, 20, SpringLayout.SOUTH, cpfTxt);
	...
	public void actionPerformed(ActionEvent e) {
		if ("Buscar".equals(e.getActionCommand())) {
			if(this.idClienteTxt.getText().compareTo("")!=0) {
				...
				if(clienteEncontrado != null) {
					...
					if (conta != null) {
						...
						this.btnConcederEmprestimo.setEnabled(true);
						...
						this.btnConcederEmprestimo.setEnabled(false);
			...
			try {
				...
				} else if ("Conceder emprestimo".equals(e.getActionCommand())){
					NumberFormat moneyFormat = NumberFormat.getCurrencyInstance();
					Banco banco = new Banco();
					//Double limiteMaximoDoCliente = banco.calcularLimiteMaximo(conta);
					Double limiteMaximoDoCliente = banco.calcularLimiteMaximo(conta, conta.getSaldo(), conta.getLimite(), banco.calcularTotalDeGastos(), banco.calcularSaldoTotal(), banco.getClientes().size(), banco.getFuncionarios().size());
					double valor = Double.valueOf(JOptionPane.showInputDialog(this, "Insira o novo limite para o cliente.\nO valor máximo permitido para este cliente é "+moneyFormat.format(limiteMaximoDoCliente)+"."));
					if(valor >= limiteMaximoDoCliente) {
						JOptionPane.showMessageDialog(this, "Limite não permitido!", "Erro", JOptionPane.ERROR_MESSAGE);
					} else {
						conta.setLimite(valor);
						contaDao.altera(conta);
						this.limiteTxt.setValue(valor);
						JOptionPane.showMessageDialog(this, "Limite aplicado com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
					}
				} else if("Salvar alterações".equals(e.getActionCommand())) {
					if(verificaCampos(this.nomeTxt.getText(),
									  this.enderecoTxt.getText(),
									  this.cpfTxt.getText())) {
						clienteEncontrado.setNome(this.nomeTxt.getText());
						clienteEncontrado.setEndereco(this.enderecoTxt.getText());
						clienteEncontrado.setCpf(this.cpfTxt.getText());
						clienteDao.altera(clienteEncontrado);
						JOptionPane.showMessageDialog(this, "Dados atualizados com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
					} else {
						JOptionPane.showMessageDialog(this, "Erro nos dados do cliente!", "Erro", JOptionPane.ERROR_MESSAGE);
					}
				}
				contaDao.altera(conta);
	...
	protected boolean verificaCampos(String nome, String endereco, String cpf) {
		boolean check = true;
		if(nome.equals("") ||
		   endereco.equals("") ||
	...
	public JButton getBtnConcederEmprestimo() {
		return btnConcederEmprestimo;
	}
